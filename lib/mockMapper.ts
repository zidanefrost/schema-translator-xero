...
// MOCK of the Mapping Engine. Pure logic — runs server-side behind /api/map
// in dev, and directly in the browser in the static GitHub Pages demo.
...
function parseDate(raw: unknown): { value: string | null; confidence: number; rationale: string } {
  ...
  const parsed = new Date(hasYear ? s : `${s} ${new Date().getFullYear()}`);
  if (!Number.isNaN(parsed.getTime())) {
    const iso = parsed.toISOString().slice(0, 10);
    return {
      value: iso,
      confidence: hasYear ? 0.9 : 0.7,
      rationale: hasYear
        ? `Normalised "${s}" to ${iso}.`
        : `"${s}" has no year — assumed the current year (${iso}).`,
    };
  }
...
export function mapRecord(
  ...
  // --- Contact resolution -------------------------------------------------
  const nameHit = findValue(record, prof, ["customer", "cust", "client", "payer", "company", "contact", "name"], ["customer name", "company name", "customer or company"]);
  ...
  if (action === "create_invoice" || action === "create_bill") {
    const dealHit = findValue(record, prof, ["deal_name", "deal", "description", "line_item", "product", "service"], ["deal name", "deal or project", "description", "product"]);
    const notesHit = findValue(record, prof, ["notes", "note", "memo", "comment"], ["notes", "memo", "free text"]);
    ...
    const amountHit = findValue(record, prof, ["amount", "amt", "value", "total", "price"], ["amount", "value in", "price"]);
    ...
    const dateHit = findValue(record, prof, ["close_date", "date", "dt_", "closed"], ["close date", "date"]);
    ...
    const emailHit = findValue(record, prof, ["email", "e-mail", "mail"], ["email"]);
    if (emailHit) {
    ...
    const invoiceHit = findValue(record, prof, ["invoice", "inv_", "reference", "ref"], ["invoice"]);
    ...
    const amountHit = findValue(record, prof, ["amount", "amt", "total", "paid"], ["amount", "payment"]);
    const amt = parseAmount(amountHit?.value);
    fields.push({
      name: "Amount",
      value: amt.value,
    ...
    const dateHit = findValue(record, prof, ["date", "paid_at", "created", "timestamp"], ["date", "time"]);
    const date = parseDate(dateHit?.value);
    fields.push({
      name: "Date",
      value: date.value,
    ...
    // create_contact
    const emailHit = findValue(record, prof, ["email", "mail"], ["email"]);
    fields.push({
      name: "Name",
      value: contact.name,
      source_field: nameHit?.key ?? null,
